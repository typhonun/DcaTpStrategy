...
class RsiDCAShort1(IStrategy):
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, current_profit: float,
        ...
        if n > 0 and last_dca_time and not reduce6_done:
            ...
            if current_time >= dca_dt + timedelta(hours=6):
        ...
        if not dca_done and current_rate >= threshold and last_rsi > 70:
            ...
            return buy_amt, f"add_short_n={n + 1}"
        ...
        if current_profit >= applicable_roi:
            ...
            if not last_tp or Timestamp(last_tp, unit='s').floor('T') != candle_ts:
                ...
                logger.info(
                    ...
                    f"{GREEN}n=({u}→{new_u}){RESET}, {YELLOW}保证金={trade.stake_amount:.8f}{RESET}, "
