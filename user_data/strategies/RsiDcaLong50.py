...
出场信号（populate_exit_trend）
不使用 exit_signal，所有平仓动作都由 adjust_trade_position 内的逻辑来完成，因此 exit_long = 0 始终不触发。

仓位动态调整（adjust_trade_position）
...
当分批止盈的第一笔（ft_order_tag=="tp30"）成交后，自动将 need_rebuy70=True，以触发“再买回70%”逻辑。
    ...
    def adjust_trade_position(
        ...
        if u > 0 and last_dca_time and not reduce6_done:
            ...
            if current_time >= dca_dt + timedelta(hours=12):
                sell_amt = -0.20 * float(trade.stake_amount)
                logger.info(
                    f"[{trade.pair}][6h DCA 后减仓20%] DCA持续 已超12h,减仓20%"
                    f"{YELLOW}保证金={trade.stake_amount:.8f}{RESET},"
        ...
        if u == 0 and not reduce8_done:
            if current_time >= open_time + timedelta(hours=16):
