...
class DcaTp(IStrategy):
    ...
    def trend_add(self, trade: Trade, last: dict, margin: float, collateral: float, current_time: datetime):
        ...
        if level == 0 and u == 0 and (not reset_needed) and price is not None and is_bullish_trend:
            ...
            amt = collateral * 0.02  # 趋势加仓数量
            _add_trend_total_usdt(abs(amt))
            avg_price = self._get_float(trade, 'dynamic_avg_entry', float(trade.open_rate or 0.0))
    ...
    def dca_add(self, trade: Trade, df: DataFrame, last: dict, current_time: datetime, current_rate: float):
        ...
        threshold = avg_entry * (1 - 0.01 - 0.02 * u)  # dca 价格阈值
    ...
    def profit_rebuy(self, trade: Trade, last: dict, collateral: float):
        ...
        buy_amt = target_value - current_value  # 浮盈加仓数量
        ...
        avg_price = self._get_float(trade, 'dynamic_avg_entry', float(trade.open_rate or 0.0))
        added_qty = self._qty_from_usdt(buy_amt, price, lev)
        prev_cost = current_qty * avg_price
        new_avg = avg_price
        if (current_qty + added_qty) > 0:
            new_avg = (prev_cost + buy_amt * lev) / (current_qty + added_qty)
        self._set(trade, 'dynamic_avg_entry', float(new_avg))
        logger.info(
            f"[{trade.pair}] {GREEN}[浮盈加仓至 5%]{RESET}, n=({n}->{n + 1}), 当前价={price:.4f}, "
            f"{YELLOW}保证金={current_value:.4f}{RESET}, 加仓={buy_amt:.4f} USDT, 新均价={new_avg:.4f}"
        )
        return float(buy_amt), "rebuy_to_5pct"
    ...
    def tp_reduce(self, trade: Trade, last: dict, current_profit: float, margin: float, current_rate: float,
        ...
        if u > 0:
            ...
            target_usdt = float(collateral_total) * 0.05  # 浮亏止盈减仓数量
            ...
            if sell_qty <= 0:
                ...
                if need_add_usdt > 0:
                    ...
                    return float(need_add_usdt), "tp_afterDCA_rebuy_to_5pct"
                return None
    ...
    def grid_single(self, trade: Trade, last: dict, current_time: datetime, current_rate: float,
        ...
        if price <= grid_lower:
            ...
            # 动态网格加仓数量
            grid_frac = max(0.01, 0.02 - 0.002 * w)
            buy_usdt = collateral * grid_frac
            if buy_usdt <= 0:
                return None
            ...
            logger.info(
                ...
                f"{YELLOW}保证金={cur_margin:.2f}{RESET}, 加仓={buy_usdt:.4f} USDT, 新均价={new_avg:.4f}, w={new_w}, "
                f"下次网格 减仓价={next_buy:.4f}, 加仓价={next_sell:.4f}"
            )
            return float(buy_usdt), f"grid_add_{grid_frac:.3f}"

        if price >= grid_upper:
            grid_sell_frac = 0.02
            sell_usdt = collateral * grid_sell_frac
            sell_usdt = min(sell_usdt, cur_margin)

            if sell_usdt <= 0:
                logger.debug(f"[{trade.pair}] 网格减仓触发，保证金不足，跳过")
                return None
            ...
            logger.info(
                f"[{trade.pair}] {RED}[网格减仓 2%]{RESET} (triggered by {price_source}), 当前价={price:.4f}, 触发价={grid_upper:.4f}, "
                f"{YELLOW}保证金={cur_margin:.2f}{RESET}, 减仓={sell_usdt:.4f}, w={new_w}{RESET}, "
                f"下次网格 加仓价={next_buy:.4f}, 减仓价={next_sell:.4f}"
            )
