...
class DcaTp(IStrategy):
    ...
    minimal_roi_user_defined = {  # 止盈参数
        "300": 0.010, "290": 0.020, "280": 0.030, "270": 0.040, "260": 0.050,"250": 0.060, "240": 0.080, "230": 0.090, "220": 0.100, "210": 0.110,"200": 0.120, "190": 0.130,
        "180": 0.140, "160": 0.145, "140": 0.150,"120": 0.155, "100": 0.160, "80": 0.165,
        "60": 0.170, "50": 0.175,"40": 0.180, "30": 0.185, "20": 0.190, "10": 0.195, "0": 0.200,
    }
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, current_profit: float,
        ...
        if level == 0 and not reset_needed and is_bullish_trend:
            ...
            amt = collateral_add(0.03)  # 趋势加仓参数
            logger.info(f"{GREEN}[{trade.pair}] 多头趋势加仓总资金 3%{RESET} 保证金={margin:.4f}, 加仓={abs(amt):.4f} USDT")
            return amt, 'trend_add30_bull'
        # KDJ 衰弱减仓
        ...
        if not trade.get_custom_data('bottom_added') and last['j_30'] < 0 and last['rsi'] < 35:  # KDJ_J&Rsi参数
            ...
            amt = collateral_add(0.02)  # 抄底加仓参数
            logger.info(
                f"{BLUE}[{trade.pair}] 抄底加仓总资金 2%: J={last['j_30']:.2f}, RSI={last['rsi']:.1f}, "
                f"保证金={margin:.4f}, 加仓={amt:.4f} USDT{RESET}"
            )
            return amt, 'bottom_add20'
        # 逃顶
        if not trade.get_custom_data('top_reduced') and current_profit > 0 and last['j_30'] > 100 and last['rsi'] > 65:  # KDJ_J&Rsi参数
