...
class DcaTpShort(IStrategy):
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if level == 2 and last['k_30'] > last['d_30']:
            ...
            logger.info(
                f"{RED}[{trade.pair}] KDJ 衰弱减仓30%{RESET}"
        ...
        #     amt = -0.5 * margin
        #     logger.info(
        #         f"{BLUE}[{trade.pair}] 多头信号，空头减仓50%{RESET} 保证金={margin:.4f}, 减仓={abs(amt):.4f} USDT"
        #     )
        ...
        # 抄顶
        if not trade.get_custom_data('top_added') and last['j_30'] > 100 and last['rsi'] > 70:  # KDJ_J&Rsi参数
        ...
        # 逃底
        if not trade.get_custom_data('bottom_reduced') and last['j_30'] < 0 and last['rsi'] < 35:  # KDJ_J&Rsi参数
            trade.set_custom_data('bottom_reduced', True)
            amt = -0.6 * margin
            logger.info(
                f"{RED}[{trade.pair}] 逃底减仓60%: J={last['j_30']:.2f}, RSI={last['rsi']:.1f}, "
                f"保证金={margin:.4f}, 减仓={abs(amt):.4f} USDT{RESET}"
            )
            return amt, 'bottom_cover60_short'
