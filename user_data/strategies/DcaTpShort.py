...
class DcaTpShort(IStrategy):
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if level == 0 and not reset_needed and is_bearish_trend:
            ...
            logger.info(
                f"{GREEN}[{trade.pair}] 空头趋势加空 3% {RESET}"
                f"保证金={margin:.4f}, 加空={abs(amt):.4f} USDT{RESET}"
            )
            return amt, 'trend_add30_bear'
        # KDJ 衰弱减仓
        ...
        if last_tp_price and not repull_done and price >= last_tp_price * 1.01:
            ...
            logger.info(
                f"{GREEN}[{trade.pair}] 止盈反弹加仓总资金 3%: "
        ...
        if not trade.get_custom_data('top_added') and last['j_30'] > 100 and last['rsi'] > 65:  # KDJ_J&Rsi参数
            ...
            logger.info(
                f"{BLUE}[{trade.pair}] 抄顶加空 2%: J={last['j_30']:.2f}, RSI={last['rsi']:.1f}, "
                f"保证金={margin:.4f}, 加空={abs(amt):.4f} USDT{RESET}"
            )
        ...
        if trade.get_custom_data('bottom_reduced') and price >= mid:
            ...
            logger.info(
                f"{GREEN}[{trade.pair}] 逃底反弹加仓 3%: 当前价={price:.4f}, 布林中轨={mid:.4f}, "
