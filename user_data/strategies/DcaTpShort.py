...
class DcaTpShort(IStrategy):
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if last_floating_tp_price and floating_confirmed and not floating_done and price >= last_floating_tp_price * 1.01:
            amt = collateral_add(0.02)
        ...
        if current_profit >= roi:
            if u > 0:
                ...
                try:
                    pending_price = float(current_rate) if current_rate is not None else None
                except Exception:
                    pending_price = None

                trade.set_custom_data('floating_tp_pending', True)
                trade.set_custom_data('pending_floating_tp_price', pending_price)
                trade.set_custom_data('floating_tp_repull_done', False)
                trade.set_custom_data('floating_tp_confirmed', False)
                trade.set_custom_data('dca_count', 0)
                trade.set_custom_data('tp_count', 0)
    ...
    def order_filled(self, pair: str, trade: Trade, order: Order, current_time: datetime, **kwargs) -> None:
        ...
        if tag == "tp30" and order.side == "buy":
        ...
        if order.side == "buy" and tag.startswith("tp_afterDCA"):
