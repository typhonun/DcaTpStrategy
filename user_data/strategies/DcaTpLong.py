...
class DcaTpLong(IStrategy):
    ...
    def on_trade_open(self, trade: Trade, **kwargs) -> None:
        flags = {
            ...
            'bb24h_first_done': False, 'floating_tp_confirmed': False,
            'floating_tp_pending': False, 'pending_floating_tp_price': None,
    ...
    def order_cancelled(self, pair: str, trade: Trade, order: Order, current_time: datetime, **kwargs) -> None:
        tag = getattr(order, 'ft_order_tag', None) or ''

        if tag and tag.startswith("tp_afterDCA"):
            trade.set_custom_data('floating_tp_pending', False)
            trade.set_custom_data('pending_floating_tp_price', None)
            trade.set_custom_data('floating_tp_confirmed', False)
            trade.set_custom_data('floating_tp_repull_done', False)
            logger.info(f"[{trade.pair}] tp_afterDCA order cancelled -> cleared pending flags")

    def order_rejected(self, pair: str, trade: Trade, order: Order, current_time: datetime, **kwargs) -> None:
        self.order_cancelled(pair, trade, order, current_time, **kwargs)
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, current_profit: float,
        ...
        if level == 0 and not reset_needed and is_bullish_trend:
            ...
            logger.info(f"{GREEN}[{trade.pair}] 多头趋势加仓 3%{RESET} 保证金={margin:.4f}, 加仓={abs(amt):.4f} USDT")
        ...
        #     amt = -0.5 * margin
        #     logger.info(
        #         f"{BLUE}[{trade.pair}] 空头信号止损{RESET} 保证金={margin:.4f}, 减仓={abs(amt):.4f} USDT"
        #     )
        ...
        pullback_ready = bool(trade.get_custom_data('pullback_ready'))
        if (ref is not None and pullback_ready and not pb_done
        ...
        # 当价格回落到回撤价的 0.99 时加仓
        if last_fb_price and fallback_ready and not done and price <= last_fb_price * 0.99:  # 回落价格参数
        ...
        floating_confirmed = bool(trade.get_custom_data('floating_tp_confirmed'))

        if last_floating_tp_price and floating_confirmed and not floating_done and price <= last_floating_tp_price * 0.99:
            amt = collateral_add(0.02)
        ...
        if current_profit >= roi_target:
            if u > 0:
                ...
                try:
                    price_val = float(current_rate) if current_rate is not None else None
                except Exception:
                    price_val = None

                trade.set_custom_data('floating_tp_pending', True)
                trade.set_custom_data('pending_floating_tp_price', price_val)
                trade.set_custom_data('floating_tp_repull_done', False)
                trade.set_custom_data('floating_tp_confirmed', False)
                trade.set_custom_data('dca_count', 0)
                trade.set_custom_data('tp_count', 0)
        ...
        if elapsed_minutes >= 1440 and price <= lower:
            ...
            logger.info(
                f"{BLUE}[{trade.pair}] 24h 无止盈或加仓，布林下轨加仓 ({note}): lower={lower:.4f}, price={price:.4f}, "
    ...
    def order_filled(self, pair: str, trade: Trade, order: Order, current_time: datetime, **kwargs) -> None:
        ...
        if order.side == "sell" and tag.startswith("tp_afterDCA"):
            trade.set_custom_data('last_floating_tp_price', order.price)
            trade.set_custom_data('floating_tp_repull_done', False)
            trade.set_custom_data('floating_tp_confirmed', True)
            trade.set_custom_data('floating_tp_pending', False)
            trade.set_custom_data('pending_floating_tp_price', None)
