...
此项目是基于freqtrade框架研发的策略，安装方法参考freqtrade官方文档，DcaTpLong复制到strategies目录下，config配置可供参考，docker-compose可设置端口号（默认为8000和8001）。可同时持有不同交易对的多空仓位。该版本为开发版，后续将会更新完整的实盘数据。leverage：杠杆大小，stake_amount：初始资金，tradable_balance_ratio：资金占用率。

优点：可根据风险偏好调整杠杆，止盈条件和仓位大小，可优化MACD,KDJ,EMA,RSI指标和加减仓参数。

缺点：无固定止损，最大风险为强平。趋势判断过于简单。
